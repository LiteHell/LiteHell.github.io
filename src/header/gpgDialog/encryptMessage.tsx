    ...
    return <Wrapper active={active} onCloseButtonClick={onCloseButtonClick}>
        <div className='flex flex-col'>
            ...
            <div className='flex-grow w-full px-4'>
                <textarea onFocus={selectAllTextWhenEncrypted} onClick={selectAllTextWhenEncrypted} className='w-full h-64 block read-only:bg-zinc-200' value={encrypting ? '암호화중입니다...' : message} onChange={(evt) => setMessage(evt.target.value)} readOnly={encrypted || encrypting}>

                </textarea>
            </div>
            <OrangeButton onClick={encryptOrClearText} disabled={encrypting}>{buttonText}</OrangeButton>
        </div>
    </Wrapper>
}