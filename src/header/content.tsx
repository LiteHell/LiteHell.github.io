...
export default function Content(opts: HeaderContentOptions): ReactElement {
  ...
  const toggleGPGDialog = (newValue?: boolean) => {
    setIsGPGDialogActive(newValue ?? !isGPGDialogActive);
  };
  return (
    <div
      className="p-12 pt-6 md:pt-12 align-bottom text-left md:w-full h-full flex-1 flex flex-col justify-between bg-gray-50 bg-bottom"
      id="card-title"
    >
      ...
      <div className="text-left w-full font-sans">
        ...
        <p>
          ...
          <a className="hover:text-gray-700" href={`mailto:${opts.email}`}>
            {opts.email}
          </a>
          <br />
